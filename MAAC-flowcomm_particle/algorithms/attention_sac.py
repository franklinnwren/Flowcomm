...
class AttentionSAC(object):
    ...
    def update_critic(self, sample, soft=True, logger=None, **kwargs):
        ...
        """
        obs, acs, rews, next_obs, dones, matrix_As, log_matrix_A_probs, full_obs, obs_messg, next_obs_messg = sample
        # Q loss
        next_acs = []
        next_log_pis = []
        for pi, ob in zip(self.target_policies, next_obs_messg):
            curr_next_ac, curr_next_log_pi = pi(ob, return_log_pi=True, return_hidden_state=False)
    ...
    def update_policies(self, sample, soft=True, logger=None, **kwargs):
        ...
        for a_i, pi, ob in zip(range(self.nagents), self.policies, obs_messg):
            curr_ac, probs, log_pi, pol_regs, ent = pi(
                ob, return_all_probs=True, return_log_pi=True,
                regularize=True, return_entropy=True, return_hidden_state=False)
        ...
        Q_mean = Q_mean / self.nagents
        return matrix_As, log_matrix_A_probs, Q_mean, full_obs
